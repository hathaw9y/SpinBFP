...
def R4_rotate_down_proj_weights(layer, had_dim=-1):
    ...
    apply_exact_had_to_linear(
        W, had_dim=had_dim, output=False
...
def rotate_model(model, args):
    ...
    for idx, layer in enumerate(
        ...
        R4_rotate_down_proj_weights(layers[idx], had_dim=args.w_down_had_groupsize)
...
class QKRotationWrapper(torch.nn.Module):
    def __init__(self, func, config, *args, **kwargs):
        ...
        if kwargs is not None:
            ...
            self.k_bfp_groupsize = kwargs["k_bfp_groupsize"]
            self.qk_had_groupsize = kwargs["qk_had_groupsize"]
            self.k_quantizer.configure(
                ...
                bfp_groupsize=self.k_bfp_groupsize,
            )

    def forward(self, *args, **kwargs):
        q, k = self.func(*args, **kwargs)
        dtype = q.dtype
        if self.qk_had_groupsize > 0:
            q = matmul_block_hadU_cuda(q.float(), self.qk_had_groupsize).to(dtype)
            k = matmul_block_hadU_cuda(k.float(), self.qk_had_groupsize).to(dtype)
        else:
