...
def matmul_hadU_cuda(X, hadK, K):
    if fast_hadamard_transform is None:
        raise ImportError("fast_hadamard_transform is required for CUDA Hadamard kernels")
    n = X.shape[-1]
    if K == 1:
...
def apply_exact_had_to_linear(module, had_dim=-1, output=False):
    if fast_hadamard_transform is None:
        raise ImportError("fast_hadamard_transform is required for apply_exact_had_to_linear")
