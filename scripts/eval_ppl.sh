...
W_DOWN_HAD_GROUP_SIZE=${W_DOWN_HAD_GROUP_SIZE:-0}
QK_HAD_GROUP_SIZE=${QK_HAD_GROUP_SIZE:-0}
...
BFP_GPTQ_WEIGHT_PATH=${BFP_GPTQ_WEIGHT_PATH:-}
if [[ -n "$BFP_GPTQ_WEIGHT_PATH" ]]; then
  EXTRA_ARGS+=(--bfp-gptq-weight-path "$BFP_GPTQ_WEIGHT_PATH")
fi
python eval_ppl.py \
  --model "$MODEL" \
